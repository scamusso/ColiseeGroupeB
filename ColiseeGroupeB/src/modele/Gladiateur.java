...
public abstract class Gladiateur {
	...
	public String messageVictime(Gladiateur agresseur, Arme arme) throws Exception {
		...
		if ((agresseur.getForce() + arme.getPuissanceOffensive() - defArme)>0){
			return getNom() +" tu� par "+ agresseur.getNom()+" causant "+((agresseur.getForce()+arme.getPuissanceOffensive()) - defArme)+" d�gats";
		}else{
			return getNom() +" touch� par "+ agresseur.getNom()+" causant "+(agresseur.getForce() + arme.getPuissanceOffensive() - defArme)+" d�gats";
		}
	}
